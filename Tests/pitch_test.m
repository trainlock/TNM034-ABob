...
avgLineDist = (d(2)+d(1))/2 + (n(2)-n(1))/2; %(lineIndices(end)-lineIndices(1))/(length(lineIndices)-1);
%avgLineDist = (lineIndices(end)-lineIndices(1))/(length(lineIndices)-1);
pitchDist = round(avgLineDist/2);
...
positions = uint8(zeros(1, length(encoding)));

% Set positions: Manually to achieve an as high accuracy as possible
% TODO: CODE THIS!!!
positions(10) = centerLine;
positions(11:end) = (centerLine - pitchDist : -pitchDist : centerLine-10*pitchDist) 
%%
